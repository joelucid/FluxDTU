...
void HoymilesRadio_NRF::init(SPIClass* initialisedSpiBus, const uint8_t pinCE, const uint8_t pinIRQ)
{
    ...
    ESP_LOGI(TAG, "NRF: Cold-start reset sequence");
    ...
    if (!_radio->begin(_spiPtr.get())) {
        ESP_LOGE(TAG, "NRF: Init failed!!");
        return;
    }
...
bool HoymilesRadio_NRF::sendEsbPacket(CommandAbstract& cmd)
{
    ...
    openWritingPipe(s);
    _radio->setRetries(5, cmd.getHardwareRetryCount());
