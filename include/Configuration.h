...
#define MQTT_MAX_PAYLOAD_STRLEN 64
...
#define POWERLIMITER_FLEXIBLE_LOAD_MAX_COUNT 4

#define ZENDURE_MAX_SERIAL_STRLEN 8
#define FLEXIBLE_LOAD_MAX_NAME_STRLEN 31
...
struct POWERLIMITER_FLEXIBLE_LOAD_CONFIG_T {
    ...
    bool MqttRetain;
    char PowerMqttTopic[MQTT_MAX_TOPIC_STRLEN + 1];
    char PowerMqttJsonPath[MQTT_MAX_JSON_PATH_STRLEN + 1];

    enum PowerUnit { Watts = 0, MilliWatts = 1, KiloWatts = 2 };
    PowerUnit PowerMqttUnit;

    bool PowerMqttSignInverted;
    uint8_t StartBatterySoCThreshold;
    bool StartOnBmsChargeCurrentLimit;
    float BmsChargeCurrentLimitMargin;
    uint16_t StartPowerDemand;
    uint16_t StartDelay;
    uint16_t StartupGrace;
    uint16_t MinRuntime;
    uint16_t MinOffTime;
...
struct POWERLIMITER_CONFIG_T {
    ...
    bool TargetPowerConsumptionFollowStorageTarget;
...
struct GRID_CHARGER_CONFIG_T {
    ...
    uint16_t AutoPowerSocPlanningMinimumPowerLimit;
    float AutoPowerSocPlanningCatchUpMultiplier;
    float AutoPowerTargetPowerConsumption;
    bool AutoPowerTargetPowerConsumptionDynamicEnabled;
    uint16_t AutoPowerTargetPowerConsumptionDynamicMax;
    float AutoPowerTargetPowerConsumptionDynamicMultiplier;
    uint16_t AutoPowerTargetPowerConsumptionDynamicWindow;
