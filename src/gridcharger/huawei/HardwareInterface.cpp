...
uint32_t HardwareInterface::normalizeValueId(uint32_t valueId)
{
    // Some Huawei responses use a changing high nibble in the first payload byte
    // while the low 28 bits still carry the expected register/counter payload.
    return valueId & 0x0FFFFFFF;
}
...
bool HardwareInterface::readBoardProperties(can_message_t const& msg)
{
    ...
    uint32_t const valueId = normalizeValueId(msg.valueId);
    uint16_t counter = static_cast<uint16_t>(valueId >> 16);
    ...
    appendAscii(valueId >>  8);
    appendAscii(valueId >>  0);
...
bool HardwareInterface::readDeviceConfig(can_message_t const& msg)
{
    ...
    if (counter == 1) {
        ...
        _upData->add<DataPointLabel::Row>(static_cast<uint8_t>((valueId >> 8) & 0xFF));
        _upData->add<DataPointLabel::Slot>(static_cast<uint8_t>(valueId & 0xFF));
...
bool HardwareInterface::readRectifierState(can_message_t const& msg) const
{
    ...
    uint32_t valueId = normalizeValueId(msg.valueId);
