...
static double roundedFloatForJson(float val)
{
    return static_cast<int>(val * 100 + (val > 0 ? 0.5 : -0.5)) / 100.0;
}

static char const* flexibleLoadDefaultName(size_t index)
{
    switch (index) {
        case 0: return POWERLIMITER_FLEXIBLE_LOAD_NAME " 1";
        case 1: return "Freezer Battery";
        case 2: return POWERLIMITER_FLEXIBLE_LOAD_NAME " 3";
        case 3: return POWERLIMITER_FLEXIBLE_LOAD_NAME " 4";
        default: return POWERLIMITER_FLEXIBLE_LOAD_NAME;
    }
}

static char const* flexibleLoadDefaultTopic(size_t index)
{
    if (index == 1) { return "freezer_battery/pv_available/set"; }
    return POWERLIMITER_FLEXIBLE_LOAD_MQTT_TOPIC;
}

static bool flexibleLoadDefaultRetain(size_t index)
{
    if (index == 1) { return true; }
    return POWERLIMITER_FLEXIBLE_LOAD_MQTT_RETAIN;
}

static uint8_t flexibleLoadDefaultPriority(size_t index)
{
    return static_cast<uint8_t>(POWERLIMITER_FLEXIBLE_LOAD_PRIORITY + index);
}

static uint8_t deserializeFlexibleLoadPriority(JsonObject const& source, size_t index)
{
    int priority = source["priority"] | flexibleLoadDefaultPriority(index);
    if (priority < 1) { priority = 1; }
    if (priority > POWERLIMITER_FLEXIBLE_LOAD_MAX_COUNT) {
        priority = POWERLIMITER_FLEXIBLE_LOAD_MAX_COUNT;
    }
    return static_cast<uint8_t>(priority);
}

static void serializeFlexibleLoadConfig(PowerLimiterFlexibleLoadConfig const& source, JsonObject& target)
{
    ...
    target["mqtt_retain"] = source.MqttRetain;
    target["power_topic"] = source.PowerMqttTopic;
    target["power_json_path"] = source.PowerMqttJsonPath;
    target["power_unit"] = source.PowerMqttUnit;
    target["power_sign_inverted"] = source.PowerMqttSignInverted;
    target["start_battery_soc_threshold"] = source.StartBatterySoCThreshold;
    target["start_on_bms_charge_current_limit"] = source.StartOnBmsChargeCurrentLimit;
    target["bms_charge_current_limit_margin"] = roundedFloatForJson(source.BmsChargeCurrentLimitMargin);
    target["start_power_demand"] = source.StartPowerDemand;
    target["start_delay"] = source.StartDelay;
    target["startup_grace"] = source.StartupGrace;
    target["min_runtime"] = source.MinRuntime;
    target["min_offtime"] = source.MinOffTime;
...
static void deserializeFlexibleLoadConfig(
        JsonObject const& source,
        PowerLimiterFlexibleLoadConfig& target,
        size_t index)
{
    strlcpy(target.Name,
            source["name"] | flexibleLoadDefaultName(index),
            sizeof(target.Name));
    ...
    strlcpy(target.MqttOnPayload,
            source["mqtt_on_payload"] | POWERLIMITER_FLEXIBLE_LOAD_MQTT_ON_PAYLOAD,
            sizeof(target.MqttOnPayload));
    strlcpy(target.MqttOffPayload,
            source["mqtt_off_payload"] | POWERLIMITER_FLEXIBLE_LOAD_MQTT_OFF_PAYLOAD,
            sizeof(target.MqttOffPayload));
    target.MqttRetain = source["mqtt_retain"] | flexibleLoadDefaultRetain(index);
    strlcpy(target.PowerMqttTopic,
            source["power_topic"] | POWERLIMITER_FLEXIBLE_LOAD_POWER_MQTT_TOPIC,
            sizeof(target.PowerMqttTopic));
    strlcpy(target.PowerMqttJsonPath,
            source["power_json_path"] | POWERLIMITER_FLEXIBLE_LOAD_POWER_MQTT_JSON_PATH,
            sizeof(target.PowerMqttJsonPath));
    target.PowerMqttUnit = source["power_unit"] | PowerLimiterFlexibleLoadConfig::PowerUnit::Watts;
    target.PowerMqttSignInverted = source["power_sign_inverted"] |
            (source["sign_inverted"] | POWERLIMITER_FLEXIBLE_LOAD_POWER_MQTT_SIGN_INVERTED);
    target.StartBatterySoCThreshold = source["start_battery_soc_threshold"] | POWERLIMITER_FLEXIBLE_LOAD_START_BATTERY_SOC_THRESHOLD;
    target.StartOnBmsChargeCurrentLimit = source["start_on_bms_charge_current_limit"] | POWERLIMITER_FLEXIBLE_LOAD_START_ON_BMS_CHARGE_CURRENT_LIMIT;
    target.BmsChargeCurrentLimitMargin = source["bms_charge_current_limit_margin"] | POWERLIMITER_FLEXIBLE_LOAD_BMS_CHARGE_CURRENT_LIMIT_MARGIN;
    auto const legacyStartPowerTolerance = source["start_power_tolerance"] | POWERLIMITER_FLEXIBLE_LOAD_START_POWER_DEMAND;
    target.StartPowerDemand = source["start_power_demand"] | legacyStartPowerTolerance;
    target.StartDelay = source["start_delay"] | POWERLIMITER_FLEXIBLE_LOAD_START_DELAY;
    target.StartupGrace = source["startup_grace"] | POWERLIMITER_FLEXIBLE_LOAD_STARTUP_GRACE;
    target.MinRuntime = source["min_runtime"] | POWERLIMITER_FLEXIBLE_LOAD_MIN_RUNTIME;
    target.MinOffTime = source["min_offtime"] | POWERLIMITER_FLEXIBLE_LOAD_MIN_OFFTIME;
...
void ConfigurationClass::serializePowerLimiterConfig(PowerLimiterConfig const& source, JsonObject& target)
{
    ...
    target["target_power_consumption_follow_storage_target"] = source.TargetPowerConsumptionFollowStorageTarget;
    ...
    for (size_t i = 0; i < POWERLIMITER_FLEXIBLE_LOAD_MAX_COUNT; ++i) {
        JsonObject flexibleLoad = flexibleLoads.add<JsonObject>();
        serializeFlexibleLoadConfig(source.FlexibleLoads[i], flexibleLoad);
    }

    JsonObject flexibleLoad = target["flexible_load"].to<JsonObject>();
    serializeFlexibleLoadConfig(source.FlexibleLoads[0], flexibleLoad);
...
void ConfigurationClass::serializeGridChargerConfig(GridChargerConfig const& source, JsonObject& target)
{
    ...
    target["auto_power_soc_planning_minimum_power_limit"] = source.AutoPowerSocPlanningMinimumPowerLimit;
    target["auto_power_soc_planning_catch_up_multiplier"] = source.AutoPowerSocPlanningCatchUpMultiplier;
    target["target_power_consumption"] = source.AutoPowerTargetPowerConsumption;
    target["target_power_consumption_dynamic_enabled"] = source.AutoPowerTargetPowerConsumptionDynamicEnabled;
    target["target_power_consumption_dynamic_max"] = source.AutoPowerTargetPowerConsumptionDynamicMax;
    target["target_power_consumption_dynamic_multiplier"] = source.AutoPowerTargetPowerConsumptionDynamicMultiplier;
    target["target_power_consumption_dynamic_window"] = source.AutoPowerTargetPowerConsumptionDynamicWindow;
}
...
void ConfigurationClass::deserializePowerLimiterConfig(JsonObject const& source, PowerLimiterConfig& target)
{
    ...
    target.TargetPowerConsumptionFollowStorageTarget = source["target_power_consumption_follow_storage_target"] | POWERLIMITER_TARGET_POWER_CONSUMPTION_FOLLOW_STORAGE_TARGET;
    ...
    for (size_t i = 0; i < POWERLIMITER_FLEXIBLE_LOAD_MAX_COUNT; ++i) {
        deserializeFlexibleLoadConfig(empty, target.FlexibleLoads[i], i);
    }

    if (source["flexible_loads"].is<JsonArray>()) {
        JsonArray flexibleLoads = source["flexible_loads"].as<JsonArray>();
        for (size_t i = 0; i < POWERLIMITER_FLEXIBLE_LOAD_MAX_COUNT; ++i) {
            JsonObject flexibleLoad = flexibleLoads[i];
            if (flexibleLoad.isNull()) { continue; }
            deserializeFlexibleLoadConfig(flexibleLoad, target.FlexibleLoads[i], i);
        }
    } else {
        JsonObject flexibleLoad = source["flexible_load"];
        if (!flexibleLoad.isNull()) {
            deserializeFlexibleLoadConfig(flexibleLoad, target.FlexibleLoads[0], 0);
        }
    }
...
void ConfigurationClass::deserializeGridChargerConfig(JsonObject const& source, GridChargerConfig& target)
{
    ...
    target.AutoPowerSocPlanningCatchUpMultiplier = source["auto_power_soc_planning_catch_up_multiplier"] | GRIDCHARGER_AUTO_POWER_SOC_PLANNING_CATCH_UP_MULTIPLIER;
    target.AutoPowerTargetPowerConsumption = source["target_power_consumption"] | GRIDCHARGER_AUTO_POWER_TARGET_POWER_CONSUMPTION;
    target.AutoPowerTargetPowerConsumptionDynamicEnabled = source["target_power_consumption_dynamic_enabled"] | GRIDCHARGER_AUTO_POWER_TARGET_POWER_CONSUMPTION_DYNAMIC_ENABLED;
    target.AutoPowerTargetPowerConsumptionDynamicMax = source["target_power_consumption_dynamic_max"] | GRIDCHARGER_AUTO_POWER_TARGET_POWER_CONSUMPTION_DYNAMIC_MAX;
    target.AutoPowerTargetPowerConsumptionDynamicMultiplier = source["target_power_consumption_dynamic_multiplier"] | GRIDCHARGER_AUTO_POWER_TARGET_POWER_CONSUMPTION_DYNAMIC_MULTIPLIER;
    target.AutoPowerTargetPowerConsumptionDynamicWindow = source["target_power_consumption_dynamic_window"] | GRIDCHARGER_AUTO_POWER_TARGET_POWER_CONSUMPTION_DYNAMIC_WINDOW;
}
...
bool ConfigurationClass::read()
{
    ...
    // as the downstream project was in use a long time without the version
    // marker specific to FluxDTU, we must distinguish the cases (1) where a
...
void ConfigurationClass::migrateOnBattery()
{
    File f = LittleFS.open(CONFIG_FILENAME, "r", false);
    if (!f) {
        ESP_LOGE(TAG, "Failed to open file, cancel FluxDTU migration");
        return;
    }
    ...
    const DeserializationError error = deserializeJson(doc, f);
    if (error) {
        ESP_LOGE(TAG, "Failed to read file, cancel FluxDTU "
    ...
    if (config.Cfg.VersionOnBattery < 1) {
        ...
        // respective legacy setting is even present, as FluxDTU
        // config version 0 identifies multiple different legacy versions of
        // FluxDTU-specific settings, i.e., all before the FluxDTU config
        // version value was introduced.
    ...
    if (config.Cfg.VersionOnBattery < 8) {
        ...
        config.GridCharger.AutoPowerTargetPowerConsumptionDynamicMultiplier = huawei["target_power_consumption_dynamic_multiplier"] | GRIDCHARGER_AUTO_POWER_TARGET_POWER_CONSUMPTION_DYNAMIC_MULTIPLIER;
        config.GridCharger.AutoPowerTargetPowerConsumptionDynamicWindow = huawei["target_power_consumption_dynamic_window"] | GRIDCHARGER_AUTO_POWER_TARGET_POWER_CONSUMPTION_DYNAMIC_WINDOW;
