...
void WebApiDtuClass::onDtuAdminPost(AsyncWebServerRequest* request)
{
    ...
    const bool nrfEnabled = Hoymiles.getRadioNrf()->isInitialized();
#ifndef HOYMILES_NRF_ONLY
    const bool cmtEnabled = Hoymiles.getRadioCmt()->isInitialized();
#endif

    if (!(root["serial"].is<String>() && root["pollinterval"].is<uint32_t>())
        || (nrfEnabled && !root["nrf_palevel"].is<uint8_t>())
#ifndef HOYMILES_NRF_ONLY
        || (cmtEnabled && !(root["cmt_palevel"].is<int8_t>()
    ...
    if (nrfEnabled && root["nrf_palevel"].as<uint8_t>() > 3) {
...
#ifndef HOYMILES_NRF_ONLY
    if (cmtEnabled && (root["cmt_palevel"].as<int8_t>() < -10 || root["cmt_palevel"].as<int8_t>() > 20)) {
        retMsg["message"] = "Invalid power level setting!";
        retMsg["code"] = WebApiError::DtuInvalidPowerLevel;
        WebApi.sendJsonResponse(request, response, __FUNCTION__, __LINE__);
        return;
    ...
    if (cmtEnabled && root["cmt_country"].as<uint8_t>() >= CountryModeId_t::CountryModeId_Max) {
