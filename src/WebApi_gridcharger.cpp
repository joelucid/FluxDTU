...
void WebApiGridChargerClass::onAdminPost(AsyncWebServerRequest* request)
{
    ...
    if (!(root["enabled"].is<bool>()) ||
        ...
        !(root["auto_power_soc_planning_minimum_power_limit"].is<uint16_t>()) ||
        !(root["auto_power_soc_planning_catch_up_multiplier"].is<float>()) ||
        !(root["target_power_consumption_dynamic_enabled"].is<bool>()) ||
        !(root["target_power_consumption_dynamic_max"].is<uint16_t>()) ||
        !(root["target_power_consumption_dynamic_multiplier"].is<float>()) ||
        !(root["target_power_consumption_dynamic_window"].is<uint16_t>())) {
    ...
    auto failValidation = [&](String const& message) {
    ...
    auto const catchUpMultiplier = root["auto_power_soc_planning_catch_up_multiplier"].as<float>();
    if (catchUpMultiplier < 0.0f || catchUpMultiplier > 10.0f) {
        failValidation("Invalid SoC planning catch-up multiplier!");
        return;
    }
