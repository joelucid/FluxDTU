...
void WebApiWsLiveClass::generateOnBatteryJsonResponse(JsonVariant& root, bool all)
{
    ...
    if (all
            || (gridChargerStats->getLastUpdate() - _lastPublishGridCharger) < halfOfAllMillis
            || (config.GridCharger.AutoPowerEnabled
                && (PowerMeter.getLastUpdate() - _lastPublishGridCharger) < halfOfAllMillis)) {
    ...
    if (all
            || (PowerMeter.getLastUpdate() - _lastPublishPowerLimiter) < halfOfAllMillis
            || (FlexibleLoadStats.getLastUpdateMillis() - _lastPublishPowerLimiter) < halfOfAllMillis) {
        auto powerLimiterObj = root["powerlimiter"].to<JsonObject>();
        ...
        if (config.PowerLimiter.Enabled) {
            auto serializeFlexibleLoad = [](JsonObject& flexibleLoadObj,
                    PowerLimiterFlexibleLoadConfig const& flexibleLoadConfig,
                    size_t index) {
                flexibleLoadObj["index"] = index;
                ...
                flexibleLoadObj["startReason"] = FlexibleLoad.getStartReasonText(index);
                flexibleLoadObj["startBlockReason"] = FlexibleLoad.getStartBlockReasonText(index);
                flexibleLoadObj["stopReason"] = FlexibleLoad.getStopReason(index);
                auto const power = FlexibleLoadStats.getPowerWatts(index);
                if (power) {
                    addTotalField(flexibleLoadObj, "power", *power, "W", 1);
                }
            ...
            auto flexibleLoads = powerLimiterObj["flexibleLoads"].to<JsonArray>();
            for (size_t i = 0; i < POWERLIMITER_FLEXIBLE_LOAD_MAX_COUNT; ++i) {
                auto flexibleLoadObj = flexibleLoads.add<JsonObject>();
                serializeFlexibleLoad(flexibleLoadObj, config.PowerLimiter.FlexibleLoads[i], i);
            }

            auto flexibleLoadObj = powerLimiterObj["flexibleLoad"].to<JsonObject>();
            serializeFlexibleLoad(flexibleLoadObj, config.PowerLimiter.FlexibleLoads[0], 0);
        }

        if (!all) { _lastPublishPowerLimiter = millis(); }
    }
}
...
void WebApiWsLiveClass::generateCommonJsonResponse(JsonVariant& root)
{
    ...
    bool radioProblem = Hoymiles.getRadioNrf()->isInitialized() && (!Hoymiles.getRadioNrf()->isConnected() || !Hoymiles.getRadioNrf()->isPVariant());
#ifndef HOYMILES_NRF_ONLY
    radioProblem = radioProblem || (Hoymiles.getRadioCmt()->isInitialized() && (!Hoymiles.getRadioCmt()->isConnected()));
#endif
    hintObj["radio_problem"] = radioProblem;
